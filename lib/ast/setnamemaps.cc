...
void AST_Name_Map_Visitor::visit(Program* node) {
    ...
    if (node->main) {
        ...
        Formal* return_formal = new Formal(nullptr, new Type(nullptr), new IdExp(nullptr, "_^return^_main"));
        name_maps->add_method_formal("_^main^_", "main", "_^return^_main", return_formal);
        name_maps->add_method_formal_list("_^main^_", "main", vector<string>());
...
void AST_Name_Map_Visitor::visit(MethodDecl* node) {
    ...
    for (auto ancestor : ancestors) {
        if (name_maps->is_method(ancestor, method_name)) {
            vector<Formal*>* ancestor_fl = name_maps->get_method_formal_list_ast(ancestor, method_name);
            Formal* return_formal = name_maps->get_method_formal(ancestor, method_name, "_^return^_"+method_name);
            if (node->fl) {
                // 检查参数数量
    ...
    if (node->fl) {
        ...
        // 添加返回值在参数列表里，但不加在formal_list中
        Formal* return_formal = new Formal(nullptr, node->type, new IdExp(nullptr, "_^return^_"+current_method));
        name_maps->add_method_formal(current_class, current_method, "_^return^_"+current_method, return_formal);
    }
    if (node->vdl) {
        for (auto* var_decl : *node->vdl) {
            var_decl->accept(*this);
